...
## Forma Studio (프로젝트 전용)

Forma Studio 에이전시 웹사이트 전용 컴포넌트. Groth Studio 레퍼런스 기반.

### UI Elements (Atoms)

- StatusTag: 프로젝트 상태 태그. IN PROGRESS/COMPLETED 등. variant, size 지원 (`components/forma/StatusTag.jsx`)
- DottedDivider: 점선 구분선. 건축 도면 스타일. orientation, spacing 지원 (`components/forma/DottedDivider.jsx`)
- TimezoneClock: 시간대 시계 표시. city, timezone props. 실시간 업데이트 (`components/forma/TimezoneClock.jsx`)

### Cards (Molecules)

- ProjectCard: 프로젝트 카드. 이미지 + 상태태그 + 제목(세리프) + 설명 + 메타정보 (`components/forma/ProjectCard.jsx`)
- ResearchCard: 리서치 카드. 사이드바용 소형 카드. 이미지 + 제목 + 설명 + 링크 (`components/forma/ResearchCard.jsx`)

### Layout

- TwoColumnLayout: 2컬럼 레이아웃. 메인콘텐츠(~75%) + 고정사이드바(~25%). sidebarPosition, sidebarWidth 지원 (`components/forma/TwoColumnLayout.jsx`)

### Navigation

- FormaHeader: Forma Studio 헤더. 로고 + 시간대표시 + 네비게이션 메뉴 (`components/forma/FormaHeader.jsx`)
- FormaFooter: Forma Studio 푸터. 다크올리브 배경 + 로고 + 네비게이션 + 저작권 (`components/forma/FormaFooter.jsx`)

### Sections

- HeroSection: 히어로 섹션. 대형 로고 + 브랜드 소개 텍스트 (`sections/forma/HeroSection.jsx`)
- ProjectsSection: 프로젝트 그리드 섹션. 섹션제목 + ProjectCard 그리드 (`sections/forma/ProjectsSection.jsx`)
- ResearchSidebar: Research & Design 사이드바. 테라코타 배경 + ResearchCard 리스트 (`sections/forma/ResearchSidebar.jsx`)

### Pages

- FormaHomePage: Forma Studio 홈페이지. TwoColumnLayout + 모든 섹션 통합 (`pages/forma/FormaHomePage.jsx`)
